...
bool CMySqlHPE::GetCurves(vector<Curve>& curves, CString railNo, uint8_t xingbie, double start, double end)
{
	CString strSql;
	strSql.Format("select * from curve_info where rail_no = '%s' and xingbie = %d order by s_mil", railNo, xingbie);
	MYSQL_RES* rs;
	if (!Query(strSql, &rs))
	{
		return false;
...
bool CMySqlHPE::GetBridges(vector<Bridge>& bridges, CString railNo, uint8_t xingbie, double start, double end)
{
	CString strSql;
	strSql.Format("select * from bridge_info where rail_no = '%s' and xingbie = %d order by s_mil", railNo, xingbie);
	MYSQL_RES* rs;
	if (!Query(strSql, &rs))
	{
		return false;
...
{
	CString strSql;
	strSql.Format("select * from tunnel_info where rail_no = '%s' and xingbie = %d order by s_mil", railNo, xingbie);
	MYSQL_RES* rs;
	if (!Query(strSql, &rs))
	{
		return false;
