...
bool AddWork(char* strFile, double& gps_log, double& gps_lat)
{
	...
	theApp.sql.GetBridges(vBridges, g_strRailNo, g_xingbie, 0, 0);
	theApp.sql.GetCurves(vCurves, g_strRailNo, g_xingbie, 0, 0);
	theApp.sql.GetTunnels(vTunnels, g_strRailNo, g_xingbie, 0, 0);
							...
							for (int p = 0; p < crE.size(); ++p)
							{
								...
								if (iD < iE)
								{
									...
									if (info.MaxV >= 150)
									{
										...
										sprintf_s(wound.According, "D出波，位移：%.1f大格，幅值:%d\nE出波，位移：%.1f大格，幅值:%d", 0.02f * crInfo.Shift, crInfo.MaxV, 0.02f * info.Shift, info.MaxV);
										vWounds.push_back(wound);
										break;
									}
								}
