...
LRESULT CMainFrame::OnMessageNet(WPARAM wParam, LPARAM lParam)
{
	...
	//只处理来自主控软件的信息
	CString label(temp);
	int idx = label.Find('_');
	if (idx < 0)
	{
		return 0;
	}
	CString strCode = label.Left(idx);
	CString strData = label.Mid(idx + 1);
	if (strCode == "0")
	{
	...
	{
		uint64_t wID = strtoull(temp + idx + 1, NULL, 10);
		SendWoundData(wID);
	}
...
void CMainFrame::CreateWebCom()
{
	...
	BOOL bSuccess = m_ts.InitialServerSocket(4666);
	if (!bSuccess)
	{
		return;
	}
...
void CMainFrame::SendWounds(vector<Wound_Judged>& vWounds)
{
	CString strWounds;
	if (vWounds.size() > 0)
	{
		strWounds.Format("<<<1_{data:{rail:{name:'%s',xingbie:'%s',gubie:'%s',s_mil:%.5lf, e_mil:%.5lf},detail:[", g_strRailName, g_strXingBieDefines[g_xingbie], g_strGuBieDefines[g_gubie], g_startPos, g_endPos);
		CString strTemp;
		for (int i = 0; i < vWounds.size(); ++i)
		{
		...
		*/
		...
		m_ts.Write(utf8_string, nUTF8len);
...
void DoJudge(void* param)
{
	...
	vector<int32_t> vblockBeginSteps;
	...
	while (pFrame->m_bJudging)
	{
		...
		{
			vblockBeginSteps.push_back(vBdatas[i].BlockHead.indexL);
			pFrame->m_vBlockHeads.push_back(vBdatas[i].BlockHead);
		}

		Analyse(pFrame->m_Head, vBdatas, vAFrames, vblockBeginSteps, vWounds, pFrame->m_vPMs);
		for (int i = 0; i < vWounds.size(); ++i)
		{
...
void CMainFrame::JudgeTPB(CString strTpB)
{
	StopJudge();
	Sleep(1000);
	...
	CString strSend;
	strSend.Format("<<<0_{rail:{rail_name:'%s',xingbie:'%s',gubie:'%s',s_mil:%.0lf,e_mil:%.0lf,log:%lf,lat:%lf}}>>>", g_strRailName, g_strXingbie, g_strGubie, g_startPos, g_endPos, gps_log, gps_lat);
...
LRESULT CMainFrame::ONMessageFinish(WPARAM wParam, LPARAM lParam)
{
	...
	CString str;
	str.Format("<<<2_%I64d>>>", g_FileID);
	//send(m_curSocket, (char*)(LPCTSTR)str, str.GetLength(), 0);
	m_ts.Write((char*)(LPCTSTR)str, str.GetLength());
	return 0L;
}
...
bool CMainFrame::SendWoundData(uint64_t iWoundID)
{
	...
	strData = "<<<3_" + strData + ">>>";
	const char* pSource = (const char*)(LPCTSTR)strData;
	int nLength = MultiByteToWideChar(CP_ACP, 0, pSource, -1, NULL, NULL);   // 获取缓冲区长度，再分配内存
	WCHAR *tch = new WCHAR[nLength];
	nLength = MultiByteToWideChar(CP_ACP, 0, pSource, -1, tch, nLength);     // 将MBCS转换成Unicode
	...
	char *utf8_string = new char[nUTF8len];
	WideCharToMultiByte(CP_UTF8, 0, tch, nLength, utf8_string, nUTF8len, 0, 0); //转换成UTF-8编码
	BOOL bsend = m_ts.Write(utf8_string, nUTF8len);
	delete utf8_string;
	delete tch;
