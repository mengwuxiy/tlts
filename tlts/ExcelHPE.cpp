...
//
CApplication CExcelHPE::excel_application_;
...
//打开excel文件
BOOL CExcelHPE::OpenExcelFile(LPCTSTR file_name)
{
	//先关闭
	CloseExcelFile();
...
void CExcelHPE::CloseExcelFile(BOOL if_save)
{
	//如果已经打开，关闭文件
...
void CExcelHPE::SaveasXSLFile(const CString &xls_file)
{
...
int CExcelHPE::GetSheetCount()
{
	return excel_sheets_.get_Count();
}


CString CExcelHPE::GetSheetName(long table_index)
{
	CWorksheet sheet;
...
BOOL CExcelHPE::LoadSheet(long table_index, BOOL pre_load)
{
	LPDISPATCH lpDis = NULL;
	excel_current_range_.ReleaseDispatch();
	excel_work_sheet_.ReleaseDispatch();
...
BOOL CExcelHPE::LoadSheet(LPCTSTR sheet, BOOL pre_load)
{
	LPDISPATCH lpDis = NULL;
	excel_current_range_.ReleaseDispatch();
	excel_work_sheet_.ReleaseDispatch();
...
//检查一个CELL是否是字符串
BOOL CExcelHPE::IsCellString(long irow, long icolumn)
{
	CRange range;
...
//检查一个CELL是否是数值
BOOL CExcelHPE::IsCellInt(long irow, long icolumn)
{
	CRange range;
	range.AttachDispatch(excel_current_range_.get_Item(COleVariant((long)irow), COleVariant((long)icolumn)).pdispVal, true);
	COleVariant vResult = range.get_Value2();
...
//
CString CExcelHPE::GetCellString(long irow, long icolumn)
{
...
double CExcelHPE::GetCellDouble(long irow, long icolumn)
{
	double rtn_value = 0;
	COleVariant vresult;
	//字符串
...
//VT_R8
int CExcelHPE::GetCellInt(long irow, long icolumn)
{
	int num;
	COleVariant vresult;
...
void CExcelHPE::SetCellString(long irow, long icolumn, CString new_string)
{
	COleVariant new_value(new_string);
	CRange start_range = excel_work_sheet_.get_Range(COleVariant(_T("A1")), COleVariant(_T("A1")));
...
void CExcelHPE::SetCellInt(long irow, long icolumn, int new_int)
{
...
//
void CExcelHPE::ShowInExcel(BOOL bShow)
{
...
//返回打开的EXCEL文件名称
CString CExcelHPE::GetOpenFileName()
{
	return open_excel_file_;
}

//取得打开sheet的名称
CString CExcelHPE::GetLoadSheetName()
{
	return excel_work_sheet_.get_Name();
}

//取得列的名称，比如27->AA
char *CExcelHPE::GetColumnName(long icolumn)
{
...
void CExcelHPE::SetColumnWidth(int column, long width)
{
	//char * 
	//CRange rg = excel_work_sheet_.get_Range(COleVariant(_T("A1")), COleVariant(_T("A1"))); //获取单元格区域
	//rg.put_ColumnWidth(_variant_t((long)30));  //设置列宽。
}
...
void CExcelHPE::ClearExcelValue(long iRow, long iColumn)
{
	CRange start_range = excel_work_sheet_.get_Range(COleVariant(_T("A1")), covOptional);
	CRange write_range = start_range.get_Offset(COleVariant((long)iRow - 1), COleVariant((long)iColumn - 1));
	write_range.Clear();
...
void CExcelHPE::CreateExcelFile()
{
	...
	excel_work_sheet_.AttachDispatch(excel_sheets_.get_Item(_variant_t("sheet1")), true);
}